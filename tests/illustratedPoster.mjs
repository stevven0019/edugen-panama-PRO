...
import {illustratedSheets,vocabularyWords,illustrationPrompt,posterTiles,numberRange,validTileReview} from '../src/resources/illustratedPoster.js';
...
global.fetch=async(url,options)=>{if(url.includes('identitytoolkit'))return {ok:true,json:async()=>({users:[{localId:'user'}]})};const payload=JSON.parse(options.body);if(url.includes('models/gemini-2.5-flash:')){reviewCalls++;assert.equal(payload.contents[0].parts[1].inlineData.mimeType,'image/png');const review={gridCorrect:true,tiles:posterTiles(grade5).tiles.map((e,index)=>({index,matches:reviewMode!=='mismatch',noText:true}))};return {ok:true,json:async()=>({candidates:[{content:{parts:[{text:reviewMode==='malformed'?'not json':JSON.stringify(review)}]}}]})};}imageCalls++;assert.deepEqual(payload.generationConfig.responseModalities,['TEXT','IMAGE']);assert.ok(payload.contents[0].parts[0].text.includes('scissors'));return {ok:true,json:async()=>({candidates:[{content:{parts:[{inlineData:{mimeType:'image/png',data:Buffer.from('mock-image').toString('base64')}}]}}]})};};
