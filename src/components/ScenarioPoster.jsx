  ...
  const generate=async()=>{
    ...
    try{
      ...
      for(let batchIndex=cache.current.blobs.length;batchIndex<count;batchIndex++){
        ...
        const response=await fetch('/api/scenario-poster',{method:'POST',signal:controller.current.signal,headers:{'Content-Type':'application/json',Authorization:'Bearer '+token},body:JSON.stringify({grade,index,sheetIndex:0,batchIndex,posterVersion:3})});
