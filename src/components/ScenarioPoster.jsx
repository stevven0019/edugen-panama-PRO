...
export default function ScenarioPoster({grade,scenario,index,ready,user,credits,isPremium}) {
  ...
  const savePdf=async()=>{
    try{
      ...
      if(!img)throw new Error('No hay póster para descargar.');
      {
        ...
        const info=doc.getImageProperties(data),scale=Math.min(277/info.width,400/info.height);
  ...
  return <section className="mt-5 p-5 rounded-2xl border border-teal-400/30 bg-gradient-to-br from-teal-500/10 to-indigo-500/10">
    ...
    <p className="text-xs text-slate-500 dark:text-slate-400 my-2">Un solo póster A3 con Recommended Grammatical Features y Recommended Vocabulary del grado y escenario elegidos. Cada categoría tendrá su propio espacio con ilustraciones.</p>
